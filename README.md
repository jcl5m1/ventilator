...
In the event that COVID-19 hospitalizations exhaust the availability of FDA approved ventilators.  This project documents the process of converting a low-cost CPAP (Continuous Positive Airway Pressure) blower into either a rudimentary non-invasive ventilator that could help with breathing during respiratory distress.  It also provides information to using the same parts to create a low-cost powered air purifying respirator (PAPR) which is a valuable piece of personal protective equipement (PPE) that could protect caregivers.
